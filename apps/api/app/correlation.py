...
from .models import Edge, Entity, Evidence, PublicObservation, new_id, utc_now
...
def import_public_observations(
    ...
    for obs in observations:
        ...
        evidence = Evidence(
            ...
            observed_at=obs.observed_at or utc_now(),
        )
        ...
        for url in obs.external_urls:
            ...
            store.add_edge(
                Edge(
                    ...
                    relation="links_to",
                    confidence=0.90,
                    rationale=["URL pública declarada no perfil"],
    ...
    for (left_obs, left_ent, left_ev), (right_obs, right_ent, right_ev) in combinations(accounts, 2):
        ...
        # Similar username/display name alone is not enough to create an identity link.
        # At least one stronger independent signal must exist.
        if score >= 0.45 and (shared_domain or shared_media):
            store.add_edge(
                Edge(
                    id=new_id("edge"),
                    case_id=case_id,
                    source_id=left_ent.id,
                    target_id=right_ent.id,
