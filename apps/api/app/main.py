...
from .models import (
    Assessment, Case, CaseCreate, GraphResponse, ImpersonationSignals,
    ObservationImport, RunRequest, RunResponse, TargetType,
)
...
origins = [x.strip() for x in os.getenv("VIGIL_ALLOWED_ORIGINS", "http://localhost:5173").split(",") if x.strip()]
app.add_middleware(
...
def modules() -> list[dict]:
    return [
        {"id": "public-domain", "name": "Public domain sources", "mode": "passive", "available": True},
        {"id": "public-observations", "name": "Public observation correlation", "mode": "import", "available": True},
        {"id": "impersonation", "name": "Impersonation scoring", "mode": "analysis", "available": True},
        {"id": "graph-export", "name": "JSON / GraphML export", "mode": "export", "available": True},
    ]
...
def create_case(payload: CaseCreate) -> Case:
    if not payload.scope_acknowledged:
        raise HTTPException(400, "Confirme que o caso usa fontes públicas e possui finalidade legítima.")
...
async def run_case(case_id: str, payload: RunRequest) -> RunResponse:
    ...
    if case.target_type in {TargetType.DOMAIN, TargetType.ORGANIZATION} and (not requested or "public-domain" in requested):
        a, b, c, w = await collect_domain_public_sources(store, case_id, case.target)
        entities_added += a; evidence_added += b; findings_added += c; warnings.extend(w)
        modules_run.append("public-domain")

    if case.target_type == TargetType.PUBLIC_ACCOUNT:
        warnings.append("Contas públicas são correlacionadas a partir de observações importadas; não há varredura automática de pessoas.")

    store.set_status(case_id, "ready")
    return RunResponse(
        case_id=case_id, modules_run=modules_run, entities_added=entities_added,
        evidence_added=evidence_added, findings_added=findings_added, warnings=warnings
    )
